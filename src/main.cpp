...
extern "C" void kernel_main()
{
    ...
    r.clear();
    {
        ...
        {
            tools::format_to_silent(to_display, "ADS:\n");
            {
                tools::format_to_silent(to_display, "Cfg:{:x}\n", ads1115.get_config());
                r.show();
                Timer::delay_ms(2000);
            }
            {
                tools::format_to_silent(to_display, "Lo:{:x}\n", ads1115.get_lo_threshold());
                r.show();
                Timer::delay_ms(2000);
            }
            {
                tools::format_to_silent(to_display, "Hi:{:x}\n", ads1115.get_hi_threshold());
                r.show();
                Timer::delay_ms(2000);
            }
            ...
            {
                tools::format_to_silent(to_display, "Starting to\n measure");
                r.show();
                ...
                while(measureTime.time_passed() < 20 * 1'000'000)
                {
                    if (auto res = ads1115.read_now())
                    {
                        ...
                        if (!updates || f != lastMeasured)
                        {
                            ...
                            r.clear();
                            to_display.p = {0,0};
                            tools::format_to_silent(to_display, "ADS:{:.3}v", lastMeasured);
                            r.show();
                        }
                    }else
                    {
                        r.clear();
                        to_display.p = {0,0};
                        tools::format_to_silent(to_display, "Failed to read:\n{}", res);
                        r.show();
                    }
                }
                if (auto res = ads1115.stop_continuous(); !res)
                    tools::format_to_silent(to_display, "Failed to stop\n{}", res);
            }else
            {
                tools::format_to_silent(to_display, "ADS\nfailed to run\ncontinuous mode");
            }
        }else
        {
            ...
            tools::format_to_silent(to_display, "Didn't find ADS\nat {}", (uint8_t)ads1115.get_addr());
        }
    }
    tools::format_to_silent(to_display, "\nFinished");
    r.show();
