...
private:
    ...
    friend bool operator >>(Pins p, bool &v)
    {
        bool prev = v;
        v = PinRT(p).get();
        return v != prev;
    ...
    void set_page(uint8_t page) { send_command(0xB0 | page); }
...
public:
    ...
    bool is_pressed(Pins pin) { return PinRT(pin).get() == false; }

    void switch_oled(bool on) { send_command(0xAE | on); }
